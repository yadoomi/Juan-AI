...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    if (commandName === 'key') {
      ...
      if (botStateChannel) botStateChannel.send('Key was successfully entered, AI is ready to use, type `/startai` to turn it on, type `/stopai` to turn it off.');
      await interaction.reply({ content: 'Server-wide API key has been stored successfully.', ephemeral: true });
    }
    if (commandName === 'currentkey') {
      if (!server.serverOpenAIKey) {
        return interaction.reply({ content: 'No API key has been set for this server.', ephemeral: true });
      }
      const maskedKey = server.serverOpenAIKey.slice(0, 5) + '*****' + server.serverOpenAIKey.slice(-5);
      await interaction.reply({ content: `Stored API key: \`${maskedKey}\``, ephemeral: true });
    }
    if (commandName === 'startai') {
      if (!server.serverOpenAIKey) {
        return interaction.reply({ content: 'You must set an OpenAI API key first using `/key`.', ephemeral: true });
      }
      server.botActive = true;
      console.log('Bot is now active');
      client.user.setPresence({ status: 'online', activities: [{ name: 'AI Chatting', type: 'PLAYING' }] });
      await interaction.reply({ content: 'AI has been turned on.', ephemeral: true });
      const botStateChannel = interaction.guild.channels.cache.find(
        (channel) => channel.name === 'bot-state' && channel.isTextBased()
      );
    ...
    if (commandName === 'stopai') {
      ...
      await interaction.reply({ content: 'AI has been turned off.', ephemeral: true });
      const botStateChannel = interaction.guild.channels.cache.find(
        (channel) => channel.name === 'bot-state' && channel.isTextBased()
      );
...
client.on('messageCreate', async (message) => {
  ...
  if (message.content.toLowerCase() === '.restartmemory') {
    ...
    return message.reply({ content: 'Memory storage has been reset.', ephemeral: true });
  }
  ...
  if (server.typingTimeouts[userId]) clearTimeout(server.typingTimeouts[userId]);
  server.typingTimeouts[userId] = setTimeout(async () => {
    try {
      const fullMessage = server.messageBuffer[userId].trim();
      if (!fullMessage) return;
      server.messageHistory[userId].push({ role: 'user', content: fullMessage });
      server.messageBuffer[userId] = '';
      if (server.messageHistory[userId].length > 1000) server.messageHistory[userId].shift();
      let botReply = '';
      if (fullMessage.toLowerCase().includes('search') || fullMessage.toLowerCase().includes('find')) {
