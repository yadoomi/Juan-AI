...
client.on('interactionCreate', async (interaction) => {
  ...
  try {
    if (commandName === 'key') {
      ...
      await interaction.reply({ content: 'API key has been set.', ephemeral: true });
    }

    if (commandName === 'currentkey') {
      if (!server.serverOpenAIKey) {
        return interaction.reply({ content: 'No API key set.', ephemeral: true });
      }
    ...
    if (commandName === 'startai') {
      ...
      await interaction.reply({ content: 'AI is now active.', ephemeral: false });
    }
...
// Define initializeServer function
...
client.on('messageCreate', async (message) => {
  ...
  initializeServer(guildId); // Initialize server for the guild
  const server = serverData[guildId];
  ...
  // Prepend displayName to the message content
